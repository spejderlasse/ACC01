...
def accelerometer_test(request):
    ...
    if request.method == 'POST':
        ...
        startFrequency = form.data.get('startFrequency')
        stopFrequency = form.data.get('stopFrequency')
        stepFrequency = form.data.get('stepFrequency')
        stepTime = form.data.get('stepTime')
        ...
        return render(request, 'demo_module/make_test2.html',context)
